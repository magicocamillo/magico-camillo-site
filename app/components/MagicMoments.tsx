      ...
      <div className="mx-auto max-w-7xl space-y-8 text-center text-white">
        <div className="space-y-4">
          ...
          <p className="mx-auto max-w-3xl text-lg leading-8 text-white/70">
            Una galleria che cresce insieme al progetto: aggiungi nuove foto in <code className="rounded bg-white/10 px-2 py-1 text-sm text-[#d4af37]">/public/images/gallery</code> e saranno subito visibili.
          </p>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-6">
          {images.map((src, index) => (
            <motion.button
              ...
              className={`group relative overflow-hidden rounded-[32px] border border-white/10 bg-white/5 shadow-stage backdrop-blur-xl transition-all duration-300 hover:shadow-[0_20px_60px_rgba(212,175,55,0.18)] ${getItemClass(index)}`}
            >
              <img
                src={src}
      ...
      <AnimatePresence>
        {selectedImage && (
          ...
          >
            <button
              type="button"
              className="absolute right-6 top-6 inline-flex h-12 w-12 items-center justify-center rounded-full border border-white/15 bg-black/70 text-3xl text-white transition hover:bg-black/90"
            ...
            <motion.img
              ...
              className="max-h-[90vh] max-w-[90vw] rounded-[28px] object-cover shadow-[0_30px_120px_rgba(0,0,0,0.45)]"
