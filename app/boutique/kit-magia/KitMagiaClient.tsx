...
const kitMagia = products.find(
  (item) => item.id === "kit-magia"
);

export default function KitMagiaClient() {
  if (!kitMagia) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-black px-6 text-white">
        <div className="text-center">
          ...
          <p className="mt-5 text-white/60">
            Il Kit Magia non è presente nell’elenco dei prodotti.
          </p>
        </div>
      </main>
    );
  ...
  return <KitMagiaContent product={kitMagia} />;
}

function KitMagiaContent({
  product,
}: {
  product: Product;
}) {
